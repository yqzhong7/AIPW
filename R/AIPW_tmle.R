...
#'  \code{obs_est}        \tab   Constructor                \tab     Components calculating average causal effects,
#'                                                                   including propensity scores, counterfactual predictions and efficient influence functions \cr
#'  \code{estimates}      \tab   `summary()`                \tab     Risk difference, risk ratio, odds ratio and variance-covariance matrix for SE calculation \cr
#'  \code{result}         \tab   `summary()`                \tab     A matrix contains RD, RR and OR with their SE and 95%CI \cr
#'  \code{g.plot}         \tab   `plot.p_score()`           \tab     A density plot of propensity scores by exposure status (`ggplot2::geom_density`)\cr
#'  }
#'
