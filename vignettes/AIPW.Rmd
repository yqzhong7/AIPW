...
#### 4. <a id="att"></a>Calculate average treatment effects among the treated
* ##### `stratified_fit()` fits the outcome model by exposure status while `fit()` does not. Hence, `stratified_fit()` must be used to compute ATT [(Kennedy et al. 2015)](http://www.ehkennedy.com/uploads/5/8/4/5/58450265/2015_kennedy_et_al_-_semiparametric_causal_inference_in_matched_cohort_studies.pdf)
...
As shown in the message, [tmle](https://cran.r-project.org/web/packages/tmle/index.html) only support cross-fitting in the outcome model. 
