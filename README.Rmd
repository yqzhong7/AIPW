...
The current version of AIPW package supports parallel processing using [future.apply](https://github.com/HenrikBengtsson/future.apply) package under the [future](https://github.com/HenrikBengtsson/future) framework. Simply use `plan()` to enable this feature and `set.seed()` to take care of the random number generation (RNG) problem:
